-- Fix 1: Add RLS policies for email_batches table
...
CREATE POLICY "Admins can create email batches"
ON public.email_batches
FOR INSERT
TO authenticated
WITH CHECK (is_admin_user());

CREATE POLICY "Admins can update email batches"
ON public.email_batches
FOR UPDATE
TO authenticated
USING (is_admin_user());

CREATE POLICY "Admins can delete email batches"
ON public.email_batches
FOR DELETE
TO authenticated
USING (is_admin_user());
...
-- Fix 2: Add RLS policies for invitation_signup_stats table  
CREATE POLICY "Admins can view invitation signup stats"
ON public.invitation_signup_stats
FOR SELECT
TO authenticated
USING (is_admin_user());

-- Enable RLS on invitation_signup_stats table
ALTER TABLE public.invitation_signup_stats ENABLE ROW LEVEL SECURITY;

-- Fix 3: Secure all database functions with SET search_path
