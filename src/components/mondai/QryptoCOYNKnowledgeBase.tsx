...
const QryptoCOYNKnowledgeBase = () => {
  ...
  const renderKnowledgeItems = (items: any[], knowledgeBase: string) => <div className="space-y-4">
      ...
      return <Card key={item.id} className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => handleItemClick(item)}>
            ...
            <CardContent>
              ...
              <div className="flex flex-wrap gap-1 mb-3">
                {visibleKeywords.map((keyword: string) => <Badge key={keyword} variant="secondary" className={`text-xs rounded-md px-2 py-1 transition-colors ${knowledgeBase === (isMobile ? 'COYN' : 'QryptoCOYN') ? 'bg-orange-100/70 text-orange-800 border-orange-300/70 hover:bg-orange-200/70' : 'bg-violet-100/70 text-violet-800 border-violet-300/70 hover:bg-violet-200/70'}`}>
                    {keyword}
                  </Badge>)}
                {remainingKeywords > 0 && <Badge variant="outline" className="text-xs rounded-md px-2 py-1 bg-gray-100/70 text-gray-800 border-gray-300/70">
                    +{remainingKeywords}
                  </Badge>}
              </div>
              {item.connections && item.connections.length > 0 && <div className="mb-3 pt-2 border-t">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {item.connections.slice(0, 2).map((connection: string) => <Badge key={connection} variant="outline" className="text-xs rounded-md px-2 py-1 bg-rose-100/70 text-rose-800 border-rose-300/70">
                        {connection}
                      </Badge>)}
                    {item.connections.length > 2 && <Badge variant="outline" className="text-xs rounded-md px-2 py-1 bg-gray-100/70 text-gray-800 border-gray-300/70">
