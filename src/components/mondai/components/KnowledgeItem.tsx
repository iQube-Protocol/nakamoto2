...
const KnowledgeItem = ({
  ...
  // Use different colors based on knowledge base with more translucent backgrounds and lighter text
  const baseColor = knowledgeBase === 'iQubes' ? 'blue' : 'orange';
  const keywordBgColor = knowledgeBase === 'iQubes' ? 'bg-blue-100/30 text-blue-600 hover:bg-blue-200/40' : 'bg-orange-100/30 text-orange-600 hover:bg-orange-200/40';
  const connectionBgColor = knowledgeBase === 'iQubes' ? 'bg-rose-100/30 text-rose-600 border-rose-300/40' : 'bg-purple-100/30 text-purple-600 border-purple-300/40';
  return <Card className="hover:shadow-lg transition-all duration-200 cursor-pointer hover:scale-[1.02] knowledge-content" onClick={() => onItemClick(item)}>
      ...
      <CardContent className="">
        ...
        <div className="flex flex-wrap gap-1 mb-3">
          {visibleKeywords.map((keyword: string) => <Badge key={keyword} variant="secondary" className={`text-xs rounded-md px-2 py-1 h-6 transition-colors ${keywordBgColor}`}>
              {keyword}
            </Badge>)}
          {remainingKeywords > 0 && <Badge variant="secondary" className={`text-xs rounded-md px-2 py-1 h-6 transition-colors ${keywordBgColor}`}>
              +{remainingKeywords}
            </Badge>}
        </div>
        ...
        {item.connections && item.connections.length > 0 && <div className="mb-3 pt-2 border-t">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              {item.connections.length > 2 && <Badge variant="outline" className="text-xs rounded-md px-2 py-1 bg-gray-100/30 text-gray-500 border-gray-300/40">
        ...
        <div className="flex justify-between items-center">
          <Badge variant="outline" className={`text-xs rounded-md px-2 py-1 ${knowledgeBase === 'iQubes' ? 'bg-blue-100/30 text-blue-600 border-blue-300/40' : 'bg-orange-100/30 text-orange-600 border-orange-300/40'}`}>
            {knowledgeBase}
          </Badge>
