...
const KnowledgeItem = ({ item, knowledgeBase, onItemClick }: KnowledgeItemProps) => {
        ...
        <div className="flex flex-wrap gap-1 mb-3">
          {visibleKeywords.map((keyword: string) => (
            <Badge 
              key={keyword} 
              variant="secondary" 
              className="text-xs rounded-md px-2 py-1 transition-colors bg-gray-500 text-white border-gray-300/70 hover:bg-amber-600"
            >
              {keyword}
            </Badge>
          ))}
