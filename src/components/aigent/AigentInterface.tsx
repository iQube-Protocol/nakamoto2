...
import AigentHeader from './AigentHeader';
import { useKBAIConnection } from '@/hooks/aigent/useKBAIConnection';
import { sonnerToast as toast } from '@/hooks/use-toast';
import { useIsMobile } from '@/hooks/use-mobile';

const AigentInterface: React.FC = () => {
  const isMobile = useIsMobile();
  
  // Use our custom hook for Aigent functionality
  const {
    conversationId,
    documentUpdates,
    handleAIMessage,
  ...
  useEffect(() => {
    console.log("Aigent: Initializing connection to knowledge base");
    const maxRetries = 3;
    const checkConnection = async (retryCount = 0) => {
      try {
        console.log(`Aigent: Attempt ${retryCount + 1} to connect to knowledge base...`);
        // First attempt to load knowledge items
        await fetchKnowledgeItems();
        
        console.log(`Aigent: Connection status after fetch: ${connectionStatus}`);
        ...
        if (connectionStatus === 'error' && retryCount < maxRetries) {
          ...
          setTimeout(async () => {
            console.log(`Aigent: Starting retry attempt ${retryCount + 1}...`);
            await handleManualRetry();
          }, delay);
        } else if (connectionStatus === 'connected') {
          console.log('Aigent: Successfully connected to KBAI service!');
        }
      } catch (error) {
  ...
  const initialMessage = connectionStatus === 'connected'
    ? "Hello! I'm your Aigent assistant with direct KBAI integration. I can help you learn about Web3, cryptocurrency, blockchain concepts, and more using my integrated knowledge base. I'll provide concise, easy-to-understand explanations, and can create visual diagrams when helpful. What would you like to know about today?"
    : "Hello! I'm your Aigent assistant. I'm currently using an offline knowledge base for Web3 concepts. I'll provide clear, concise explanations and can create visual diagrams for complex topics. What would you like to explore today?";
...
export default AigentInterface;
